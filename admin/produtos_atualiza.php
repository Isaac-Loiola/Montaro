...
include_once '../php/class/tipo.php';
    ...
    if($_POST){ // usuário clicou no botão atualizar
        if($_FILES['imagemfile']['name']){ // Usuário escolhei uma imagem
            // deletar a imagem atual da pasta (esta decisão, poderá ser adiada para executar após a confirmação da alteração)
            unlink('../images/products/'.$_POST['imagem_atual']); // apagá a imagem
            $nome_img = $_FILES['imagemfile']['name'];
            $tmp_img = $_FILES['imagemfile']['tmp_name'];
            $rand = rand(100001, 999999);
            $dir_img = "../images/products/".$rand.$nome_img;
    ...
    $id_get = $_GET['id'];
    $produto = new Produto();
    $prod = $produto->obterPorId($id_get);

    // selecionar a lista de tipos 
    $tp = new Tipo();
    $listaTipos = $tp->obterLista();
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
                                ...
                                <select name="id_tipo" id="id_tipo" class="form-select" required>
                                    <?php foreach($listaTipos as $tipo) :?>
                                        <option value="<?= $tipo['id']?>"
                                           <?php 
                                                if($tipo['id'] == $prod['tipo_id']) echo 'selected';
                                           ?>
                                        >
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="valor" id="valor"
                                    ...
                                    value="<?=htmlspecialchars($prod['valor'])?>">
                            </div>
                        </div>
